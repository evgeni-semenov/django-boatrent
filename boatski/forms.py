...
class CreateBookingForm(forms.ModelForm):
    class Meta:
        model = Booking
        fields = ["boat", "start_date", "end_date", "skipper_name", "skipper_email", "skipper_phone"]
        widgets = {
            'start_date': DateInput(attrs={'type': 'date'}),
            'end_date': DateInput(attrs={'type': 'date'}),
        }
    ...
    def form_valid(self, form):
        print('Booking instance:', booking)
        print('Redirecting to URL:', reverse('booking_confirmation', kwargs={'pk': booking.pk}))
        return redirect('booking_confirmation', pk=booking.pk)  # 'booking_confirmation' should match the name given in urls.py
